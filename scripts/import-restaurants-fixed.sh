...
# Import Restaurants Script (Fixed)
# This script imports restaurant data to fix the menu loading issue
# Removes auth_id field which doesn't exist in production schema
...
# Create a temporary file with auth_id removed
TEMP_FILE="/tmp/restaurants_fixed.json"
echo "🔧 Removing auth_id field from restaurants data..."

# Use jq to remove auth_id field from each restaurant
jq 'map(del(.auth_id))' "$RESTAURANTS_FILE" > "$TEMP_FILE"

echo "🔄 Importing restaurants (without auth_id)..."
